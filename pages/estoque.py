...
def _editar():
    ...
    with st.form("fep"):
        ...
        if st.form_submit_button("Salvar →",type="primary"):
            fator_antigo=float(p["fator_conversao"]); est_sec=float(p["quantidade_total_secundaria"])
            payload={"nome":ne.strip(),"categoria_id":cm.get(ce),"unidade_primaria":upe,"unidade_secundaria":use,"fator_conversao":fe,"estoque_minimo_primario":eme,"ean":eane.strip() or None,"descricao":de.strip() or None,"ativo":ate}
            # Se o fator de conversão mudou, recalcula quantidade_total_secundaria:
            # qtd_primaria = est_sec / fator_antigo  →  nova_sec = qtd_primaria * novo_fator
            if abs(fe - fator_antigo) > 1e-9 and fator_antigo > 0:
                qtd_primaria=est_sec / fator_antigo
                payload["quantidade_total_secundaria"]=qtd_primaria * fe
            atualizar_produto(p["id"],payload)
